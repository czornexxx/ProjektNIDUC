...
macierzWektorow = 0; 
tmp = size(kosztEl);                        % Funkcja odczytuje iloœæ el.
n = tmp(2);
...
tmp = 1;
for i=1 :nn                   % sprawdzenie które wektory s¹ w przedziale cenowym
    ...
    if(suma <= kasa && suma > (kasa * 90/100))  % Sprawdzenie które wektory s¹ z przedzia³u
                                                % <kasa, 90%kasy> i zapisanie tych wektorów
                                                % do macierzy
        for l=1 :n
